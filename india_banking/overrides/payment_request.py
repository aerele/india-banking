...
def validate(self, method=None):
	...
	if (
		...
		tds_amount = calculate_pr_tds(self, self.net_total)
...
def validate_payment_type(self):
	...
	) or frappe.db.get_value(self.reference_doctype, self.reference_name, "credit_to")
...
def validate_bank_account(self):
	...
	if bank_account.currency != self.currency:
		frappe.throw(
			title=_(f"The party bank account currency should be in {self.currency}."),
			msg=_(
				"{}-{}- Bank Account <a href='{}'>{}</a>".format(
					self.party_type,
					self.party,
