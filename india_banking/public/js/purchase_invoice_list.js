frappe.listview_settings["Purchase Invoice"] = {
  onload: function (listview) {
		listview.page.add_action_item(__("Bank Payment Request"), ()=>{create_bulk_request(listview, "Purchase Invoice")});
	},
};
...
const create_bulk_request = function (listview, doctype) {
  ...
  frappe.confirm(__("Create {0} {1} ?", [count_of_rows, __("Bank Payment Request")]), () => {
    if (doc_name.length == 0) {
      frappe
        .call({
          method: "india_banking.india_banking.doctype.bank_payment_request.bank_payment_request.make_bulk_bank_payment_request",
