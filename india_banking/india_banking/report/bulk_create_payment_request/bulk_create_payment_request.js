	...
	onload: function (report) {
		...
		let switch_element = frappe.query_report.page.add_inner_button(switch_icon, () => {
			frappe.set_route("query-report", "Bulk Update Payment Request", {
				...
				transaction_date: [frappe.datetime.get_today(), frappe.datetime.get_today()],
			});
		});
		if ($("#switch-button").length > 0) {
			$("#switch-button").remove();
			$(`[title="Bulk Create Payment Request"]`).parent().append(switch_element);
		} else {
			$(`[title="Bulk Create Payment Request"]`).parent().append(switch_element);
		}
		...
		report.page.add_action_item(__("Create Payment Request"), function () {
			...
			frappe.call({
				method: "india_banking.india_banking.report.bulk_create_payment_request.bulk_create_payment_request.create_bulk_payment_request",
				args: {
