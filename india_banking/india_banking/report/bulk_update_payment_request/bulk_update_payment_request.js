...
frappe.query_reports["Bulk Update Payment Request"] = {
  ...
  onload: function (report) {
    let switch_icon = `
		...
		<svg class="icon  icon-md" style="" aria-hidden="true" title="Goto Bulk Create Payment Request">
    ...
    if ($("#switch-button").length > 0) {
      ...
      $(`[title="Bulk Update Payment Request"]`)
        .parent()
    ...
    frappe.query_report.page.add_inner_button("Goto Payment Order", () => {
    ...
    report.page.add_action_item(__("Submit Payment Request"), function () {
      ...
      frappe.call({
        method:
          "india_banking.india_banking.report.bulk_update_payment_request.bulk_update_payment_request.update_bulk_payment_request",
        args: {
          docnames: docnames,
  ...
  var d = new frappe.ui.Dialog({
    ...
    primary_action: function () {
      ...
      if (data.amount > data.total) {
        frappe.throw(
          `values cannot be updated more than <b>${data.total}</b>.`
        );
      }
      frappe.call({
        method:
          "india_banking.india_banking.report.bulk_update_payment_request.bulk_update_payment_request.update_value",
        args: {
